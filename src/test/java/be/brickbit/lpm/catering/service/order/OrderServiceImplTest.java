...
public class OrderServiceImplTest {
    ...
    @Test
    public void findsByTaskId() throws Exception {
        ...
        when(orderRepository.findByOrderLinesId(task.getOrderLine().getId())).thenReturn(order);
        when(dtoMapper.map(order)).thenReturn(orderDetailDto);

        OrderDetailDto result = orderService.findByTaskId(taskId, dtoMapper);
    ...
    public void throwsExceptionOnInvalidTaskId() throws Exception {
        Long taskId = randomLong();
        expectedException.expect(ServiceException.class);
        expectedException.expectMessage(String.format("Invalid Task ID (#%s)", taskId));

        when(taskRepository.findOne(taskId)).thenReturn(null);

        orderService.findByTaskId(taskId, dtoMapper);
    }
}